...
1.	Discovery & gating
  a.	Traverse folders 1..134.
  b.	Parse all GeoJSONs in a folder → build a Map<YYYYMMDD, boolean> where the value is true if ANY feature is flagged true.
  c.	For each TIFF: extract last date in filename → require a flood entry for that date.
  d.	If no date, or no flood entry, the image is skipped.
2.	Image analysis pipeline
  a.	Raster reader (no getRGB(), no color-model conversion): stream rows directly from the Raster supporting BYTE/USHORT/INT/FLOAT/DOUBLE.
  b.	For each pixel:
    i.	Normalize channels to [0..1].
    ii.	Compute luma Y (BT.601): Y = 0.299R + 0.587G + 0.114B.
    iii.	Treat interior alpha==0 as white; border transparency ignored to avoid halo artifacts.
    iv.	Quantize to 0..255 → update 256-bin histogram; build black (Y<=19) and white (Y>=236 or interior transparent) boolean masks.
  c.	Connected components (8-connectivity) to find the largest black and white components.
  d.	Geometry:
    i.	Bounding box width/height.
    ii.	Convex hull (monotone chain); rotating calipers for Euclidean diameter.
    iii.	Perimeter/area → roundness; aspect ratio.
    iv.	Heuristic shape classification from hull vertices + roundness + aspect + fill ratio.
3.	Export
  a.	Accumulate all unique hex greys across images.
  b.	Write/overwrite CSVs (headers first, then final tables).
  c.	Log reasons for skips and a histogram at the end (during earlier versions); final v6 writes rows and prints progress.
...
•	Metrics:
o	Euclidean diameter from rotating calipers on the convex hull (tight upper bound).
o	Width, Height from the component’s axis-aligned bounding box.
o	Roundness = 4πA/P², aspect = max(width,height)/min(width,height), vertex count of hull.
...
o	Square / Rectangle / Rhombus / Parallelogram / Trapezium.
