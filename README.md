...
5. Writes `Images_All.csv`, `Summary_All.csv`, and `Skipped.csv` to the dataset root. `Summary_All.csv` now restores the detailed analytics (group means/medians/modes, effect sizes, weights, XY table) from the earlier pipeline while explicitly excluding `RAW_00000` from statistical equations.
